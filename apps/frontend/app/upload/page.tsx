...
export default function UploadPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    await fetchJson('/api/posts', {
      ...
      credentials: 'include',
      body: JSON.stringify({ caption, mediaUrl: upload.url, mediaType: isVideo ? 'video' : 'image', scheduledAt: scheduledAt || undefined }),
    });
    router.push('/');
  }
    ...
    <div className="max-w-xl mx-auto">
      <form onSubmit={onSubmit} className="space-y-4 p-6 rounded-xl border">
        ...
        <div className="flex items-center gap-2">
          ...
          <input type="datetime-local" className="rounded-md border px-3 py-2 bg-transparent" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} />
        </div>
        <button disabled={!file} className="bg-brand text-white rounded-md px-4 py-2 disabled:opacity-50">Publish</button>
      </form>
    </div>
  );
}